    ...
    for(auto it = usu.compra().begin(); it != usu.compra().end(); it++)
    {
        ...
        if(artalm)
        {
            if(artalm->stock() < it->second)
            {
                ...
                throw SinStock{*artalm};
            }
        }

        // Sexto (añadido de la P4), si la fecha de expiración del LibroDigital ha pasado, no se podrá comprar dicho ebook. Por lo tanto,
        // si ha sido añadido al carrito, quitamos dicho producto del mismo, no se vaciará el carrito.
        auto ebook = dynamic_cast<LibroDigital*>(art);

        // Si la conversión fue un éxito, quiere decir que ese artículo es un LibroDigital.
        // En caso contrario, no pasaría nada.
        if(ebook)
        {
            if(ebook->f_expir() < Fecha())
            {
                // Eliminamos el ebook de la cesta de la compra.
                auto cesta = usu.compra();
                cesta.erase(ebook);
                
                // Descontamos el importe del ebook.
                importeTotal_ -= ebook->precio();

                // Si después de eliminar el ebook, resulta ser que la cesta está vacía, lanzamos la excepción Pedido::Vacio.
                if(cesta.empty())
                    throw Pedido::Vacio{usu};
            }
        }
    }
